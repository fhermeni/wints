...
func (s *Session) NewInternship(c schema.Convention, d time.Duration, cfg config.Internships) (schema.Internship, []byte, error) {
	if s.Role() >= schema.ADMIN {
		return s.store.NewInternship(c, d, cfg)
	}
