...
func (srv *Service) Conventions() ([]internship.Convention, error) {
	sql := "select creation, studentEmail, studentFn, studentLn, studentTel, promotion, tutorEmail, tutorFn, tutorLn, tutorTel, supervisorEmail, supervisorFn, supervisorLn, supervisorTel, startTime, endTime, title, company, companyWWW, skip, gratification, foreignCountry, lab from conventions"
	...
	for rows.Next() {
		...
		rows.Scan(&at, &stuEmail, &stuFn, &stuLn, &stuTel, &prom, &tutEmail, &tutFn, &tutLn, &tutTel, &supEmail, &supFn, &supLn, &supTel, &begin, &end, &title, &cpyName, &cpyWWW, &skip, &gratification, &foreignCountry, &lab)
		...
		c := internship.Convention{Creation: at, Student: stu, Tutor: tut, Supervisor: sup, Promotion: prom, Title: title, Begin: begin, End: end, Cpy: cpy, Skip: skip, ForeignCountry: foreignCountry, Lab: lab, Gratification: gratification}
